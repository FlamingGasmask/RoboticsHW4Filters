...
class BayesFilter:
    ...
    def predict(self, action: int):
        ...
        if action == 0:
            ...
            for i in range(rows):
                for j in range(cols):
                    Belief = self.belief[i, j]
                    if Belief <= 0.0:
                        continue

                    UniDistrib = Belief * 0.25
                    ...
                    for di, dj in moves:
                        ...
                        if(
                            0 <= ni < rows
                            and 0 <= nj < cols
                            and self.env.grid[ni, nj] == 0):
                            new_belief[ni, nj] += UniDistrib * (1.0 - self.motion_noise)
                            new_belief[i, j] += UniDistrib * self.motion_noise
                        else:
                            new_belief[i, j] += UniDistrib
        else:
                ...
                for heading in range(4):
                    ...
                    for k in range(n_beams):
                        ...
                        if diff < self.measurement_tolerance:
                            heading_likelihood *= (1.0 - self.measurement_noise)
                        else:
                            heading_likelihood *= (self.measurement_noise * (1.0 / (1.0 + diff)))

                    cell_likelihood += 0.25 * heading_likelihood
